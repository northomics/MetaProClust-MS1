...
```{r}                                                                                              
intensity <- read.csv(here::here("data", "60mins_ms1","ms1_60min_intensity.csv"), row.names=1)
...
ms1_umap <- umap(log_exp %>% t(), random_state=2248)

coords<-data.frame(ms1_umap$layout, Drug = meta_drug$Drug,
                   samplename = rownames(ms1_umap$layout),
                   Concentration = meta_drug$Conc)
coords$Concentration <- factor(coords$Concentration, levels = c("H", "M", "L", "1D"))

umap_plot <- ggplot(coords, aes(x = X1, y = X2)) +
...
#write.csv(fc, here::here("data", "60mins_ms1", "ms1_fc.csv"), quote=F, row.names = T)

ms1_fc_umap <- umap(fc %>% t(), random_state=2248)

coords<-data.frame(ms1_fc_umap$layout, Drug = noctrlmeta$Drug,
                   samplename = rownames(ms1_fc_umap$layout),
                   Concentration = noctrlmeta$Conc)
coords$Concentration <- factor(coords$Concentration, levels = c("H", "M", "L"))

umap_fc_plot <- ggplot(coords, aes(x = X1, y = X2)) +
        geom_point(size=4, aes(shape = Concentration, color = Drug)) + 
...
#ggsave(filename = "../../docFigs/ms1_60mins_umap.pdf", umap_fc_plot, height = 6, width = 8, units ="in")
```
## Clustering
...
sh run_ica.sh -i 100 -t 1e-8 -n 6 -o ../../data/msms/ -l ../../data/msms/ica.log ../../data/msms/60mins_ms1/ms1_fc.csv
conda deactivate 
...
```{r message=F, warning=F}
kmedoid_labels <- read.csv(here::here("data", "60mins_ms1", "k-med_labels.csv"), header=F) %>% as.vector()
...
save(ms1_60moduleTraitCor_svd, ms1_60drugclust_svd, ms1_60drugclustboot_svd,
     file=here::here("data", "60mins_ms1", "ms1_60dedro.RData"))
```