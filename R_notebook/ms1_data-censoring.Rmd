...
```{r}
load(here::here("data", "ms1int.RData"))
library(impute)
library(pvclust)
library(tidyverse)
library(DESeq2)
...
#write.csv(high_fc %>% dplyr::select(noctrlmeta$Name), here::here("output", "ms1_openms_quart", "high_log2fc.txt"), 
#          quote=F, row.names = T)
#write.csv(med_fc %>% dplyr::select(noctrlmeta$Name), here::here("output", "ms1_openms_quart", "med_log2fc.txt"), 
#          quote=F, row.names = T)
```
...
```{r}
###
high_pca <- prcomp(high_fc %>% t(), center=F, scale=F)
...
high_coords$Concentration <- factor(high_coords$Concentration, levels = c("H", "M", "L"))
...
med_pca <- prcomp(med_fc %>% t(), center=F, scale=F)
med_sampleVals<-data.frame(med_pca$x)
med_exprVals<-data.frame(med_pca$rotation)
med_PoV <- (med_pca$sdev^2/sum(med_pca$sdev^2))*100 

med_coords<-data.frame(med_sampleVals, Drug = noctrlmeta$Drug,
                        samplename = rownames(med_sampleVals),
                        Concentration = noctrlmeta$Conc)
med_coords$Concentration <- factor(med_coords$Concentration, levels = c("H", "M", "L"))
...
(pc12high <- quick_pca(coords_df = high_coords, x_PC = 1, y_PC = 2, PoV=high_PoV, shape = "Concentration", fill = "Drug"))
#(pc23 <- quick_pca(coords_df = high_coords, x_PC = 2, y_PC = 3, shape = "Concentration", fill = "Drug"))
(pc12med <- quick_pca(coords_df = med_coords, x_PC = 1, y_PC = 2,  PoV=med_PoV, shape = "Concentration", fill = "Drug"))
...
```{r}
highkmedoid_labels <- read.csv(here::here("data", "ms1_high", "k-med_labels.csv"), header=F) %>% as.vector()

medkmedoid_labels <- read.csv(here::here("data","ms1_med", "k-med_labels.csv"), header=F) %>% as.vector()
...
# grabbing the first principal component
    ...
    fc_metadata <- fc_metadata %>% mutate(drug_conc = paste0(Drug, "-", Conc))
...
high_clustering <- autoCluster(high_fc, highkmedoid_labels, noctrlmeta)
med_clustering <- autoCluster(med_fc, medkmedoid_labels, noctrlmeta)
...
(med_sil <- ggplot(med_sil, aes(x=num, y = sil_score)) +
  geom_point(size=4, colour="#F25C54", alpha=1) +
    geom_line(size=2, alpha=0.4) +
    geom_vline(xintercept=6, size = 2, linetype="dashed", color = "#38618C") +
    xlab("Number of clusters") +
    ylab("Average Silhouette Score") +
    scale_x_continuous(breaks = 1:30) +
    theme_classic(base_size=14) +
...
clust_cols_med <- c("#DF737A", "#3444DA", "#59C9A5","#FFE66D", "#2E1F27", "#B892FF")
clust_cols_high <- c("#DF737A", "#3444DA", "#59C9A5","#FFE66D")
...
## ms1 optimal clust num = 2
med_svd <- med_clustering$bootstrap %>% as.dendrogram() %>% color_branches(k=6, col=clust_cols_med) %>%
    dendextend::set("branches_lwd", 3) 
## ms1 optimal clust num = 2
...
#pdf(here::here("figs", "ms1_highmed_tanglegram.pdf"), width=6, height=5)
...
Let's compare our censored data to our original MS1 clusters

```{r}
load(here::here("data", "ms1", "ms1_dedro.RData"))
clust_cols_ms1_svd <- c("#3444DA", "#DF737A")

## ms1 optimal clust num = 2
ms1_svd <- ms1_drugclust_svd %>% as.dendrogram() %>% color_branches(k=2, col=clust_cols_ms1_svd) %>%
    dendextend::set("branches_lwd", 3) 

compare_ms1_high <- dendlist(ms1_svd, high_svd) %>% untangle(method = "step2side")

compare_ms1_high  %>% entanglement()

#pdf(here::here("figs", "ms1_high_tanglegram.pdf"), width=6, height=5)
tanglegram(compare_ms1_high , sort=F, common_subtrees_color_lines=F,highlight_branches_lwd = F, 
           highlight_distinct_edges=F, main_left="MS1", main_right="MS1-High")
#dev.off()
compare_ms1_med <- dendlist(ms1_svd, med_svd) %>% untangle(method = "step2side")

compare_ms1_med  %>% entanglement()


#pdf(here::here("figs", "ms1_med_svd_tanglegram.pdf"), width=6, height=5)
tanglegram(compare_ms1_med , sort=F, common_subtrees_color_lines=F,highlight_branches_lwd = F, 
           highlight_distinct_edges=F, main_left="MS1", main_right="MS1-Med")
#dev.off()
```

Let's also calculate a cophenetic correlation coefficient to compare high and medium intensity data to our full MS1 feature intensity data set.

```{r}
cor_coph_med <- cor_cophenetic(ms1_svd, med_svd)
cor_coph_med
set.seed(2248)
R <- 2000
cor_coph_resultsm <- numeric(R)
dend_mixedm <- med_svd
for (i in 1:R){
  dend_mixedm <- sample.dendrogram(dend_mixedm, replace = F) #just swapping labels for a permutation p-value
  cor_coph_resultsm[i] <- cor_cophenetic(ms1_svd, dend_mixedm)
}

mean(abs(cor_coph_resultsm)>=cor_coph_med)


cor_coph_high <- cor_cophenetic(ms1_svd, high_svd)
cor_coph_high
set.seed(2248)
R <- 2000
cor_coph_resultsh <- numeric(R)
dend_mixedh <- high_svd
for (i in 1:R){
  dend_mixedh <- sample.dendrogram(dend_mixedh, replace = F) #just swapping labels for a permutation p-value
  cor_coph_resultsh[i] <- cor_cophenetic(ms1_svd, dend_mixedh)
}

mean(abs(cor_coph_resultsh)>=cor_coph_high)

cor_coph_resultsh <- cor_coph_resultsh %>% as.data.frame()
