...
```{r}
peptide_intensity <- here::here("data","msms", "ms2_intensity.csv")
...
rownames(log_exp) <- filt_peptides
rownames(imputed_pep$data) <- filt_peptides
...
noctrlmeta <- metadata  %>% filter(Drug != "NC") 
fc <- fc %>% dplyr::select(noctrlmeta$Name)
#write.csv(fc, here::here("data","msms", "ms2_fc.csv"), quote=F, row.names = T)

## FC PCA that is in manuscript
...
quick_pca <- function(coords_df, x_PC, y_PC, shape, fill){
    ...
    pca_plot <- ggplot(coords, aes_string(x = paste0('PC', xaxis), y = paste0('PC', yaxis))) +
        ...
        scale_shape_manual(values=c(19,17,15,18,3)) +
        scale_x_continuous(name=xaxislabel) + 
        scale_y_continuous(name=yaxislabel) +
        theme_bw() +
        theme(legend.position = "right", text=element_text(size=16))
...
quick_pca_newcol <- function(coords_df, x_PC, y_PC, shape, fill){
    yaxis <- y_PC
    xaxis <- x_PC
    yperc <- paste0("(", round(PoV[yaxis %>% as.numeric()] ,2), "%)")
    xperc <- paste0("(", round(PoV[xaxis %>% as.numeric()] ,2), "%)")
    ...
    pca_plot <- ggplot(coords, aes_string(x = paste0('PC', xaxis), y = paste0('PC', yaxis))) +
        geom_point(size=4, aes_string(shape = shape, color = fill)) + 
        scale_color_manual(values=c("#FFA400",  "#C43171", "#3444DA")) +
        scale_shape_manual(values=c(19,17,15,18,3)) +
        scale_x_continuous(name=xaxislabel) + 
        scale_y_continuous(name=yaxislabel) +
        theme_bw() +
...
(pc23conc <- quick_pca_newcol(coords_df = coords, x_PC = 2, y_PC = 3, shape = "Drug", fill = "Concentration"))
#ggsave(filename = "../../docFigs/ms2pc23_conc.pdf", pc23conc, width = 8, height=6, units = "in")
```
...
kmedoid_labels <- read.csv(here::here("data", "msms", "k-med_labels.csv"), header=F) %>% as.vector()
...
save(msms_moduleTraitCor_pca, msms_drugclust_pca, msms_drugclustboot_pca,
     ...
     file=here::here("data", "msms", "msms_dedro.RData"))
```
