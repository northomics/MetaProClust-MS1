---
title: "Dataset 3 (MS1 from 60 min gradient)"
output:
...
*Note: the same code is used for all MS1 feature analyses.*
...
```{r}
intensity <- read.csv(here::here("data", "Dataset3","ms1_60min_intensity.csv"), row.names=1)
...
```{r, message=F, warning=F}
## some values of 
imputed_noZero <- imputed_pep$data
...
#ggsave(filename = "../../4.mSystems/2.revisionv2/figs/dataset3_pc12_ellipse.pdf", pc12, width = 8, height = 6, units = "in")
#ggsave(filename = "../../4.mSystems/2.revisionv2/figs/dataset3_pc12_ellipse.eps", pc12, width = 8, height = 6, units = "in")

#write.csv(log_exp, here::here("data","Dataset3", "dataset3-ms1_log2_normalized.csv"), quote=F, row.names = T)

```


```{r, message=F, warning=F}
msms_umap <- umap(log_exp %>% t(), random_state=2248)

coords_umap<-data.frame(msms_umap$layout, Drug = meta_drug$Drug,
                   samplename = rownames(sampleVals),
                   Concentration = meta_drug$Conc) %>%
  mutate(Drug = case_when(Drug == "NC" ~ "DMSO",
                          TRUE ~ Drug),
...
(umap_drug<- ggplot(coords_umap, aes(x = X1, y = X2)) +
        ...
        theme_bw() +
        theme(legend.position = "right", text=element_text(size=16)))
#ggsave(filename = "../../4.mSystems/2.revisionv2/figs/dataset3_umap.pdf", umap_drug, width = 8, height = 6, units = "in")
#ggsave(filename = "../../4.mSystems/2.revisionv2/figs/dataset3_umap.eps", umap_drug, width = 8, height = 6, units = "in")
```
...
sh run_ica.sh -i 100 -t 1e-8 -n 6 -o ../data/Dataset3/ -l ../data/Dataset3/ica.log ../data/Dataset3/dataset3-ms1_log2_normalized.csv
conda deactivate 
...
```{r message=F, warning=F}
kmedoid_labels <- read.csv(here::here("data", "Dataset3", "k-med_labels.csv"), header=F) %>% as.vector()
...
We cluster drug treatments using correlation distance. To identify robust clusters, we perform 1000 bootstrap iterations. Let's only look at *SVD-derived* eigenfeatures (this is the more common way of calculating an eigenfeature).
...
drug_dend <- color_branches(as.dendrogram(drug_clust_svd), k = 4, col = clust_cols_ms1_svd)


## Uncomment to save PDF of heatmap
#pdf(here::here("..","docFigs", "ms1_kmed_hm_svd60mins.pdf"), width=9, height=6)
...
```{r}
#ms1_60moduleTraitCor_pca <- moduleTraitCor_pca
#ms1_60drugclustboot_pca <- drug_clust_boot_pca
#ms1_60drugclust_pca <- drug_clust_pca
ms1_60moduleTraitCor_svd <- moduleTraitCor_svd
ms1_60drugclustboot_svd <- drug_clust_boot_svd
ms1_60drugclust_svd <- drug_clust_svd
#4 Uncomment to save the RData file
save(ms1_60moduleTraitCor_svd, ms1_60drugclust_svd, ms1_60drugclustboot_svd,
     file=here::here("data", "Dataset3", "Dataset3-ms1_60dedro.RData"))
```