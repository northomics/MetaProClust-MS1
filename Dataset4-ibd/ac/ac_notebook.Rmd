...
(umap_plot_date <- ggplot(coords_umap, aes(x = X1, y = X2)) +
    ...
    labels = format(labels, "%m/%d/%y")
        ...
        theme_bw() +
        theme(legend.position = "right", text=element_text(size=16)))
#ggsave(filename = "../../../4.mSystems/2.revisionv2/figs/umap-ac-diagnosis_update.pdf", umap_plot_diagnosis, width = 8, height = 6, units = "in")
#ggsave(filename = "../../../4.mSystems/2.revisionv2/figs/umap-ac-diagnosis_update.eps", umap_plot_diagnosis, width = 8, height = 6, units = "in")
#ggsave(filename = "../../../4.mSystems/2.revisionv2/figs/umap-ac-date_update.pdf", umap_plot_date, width = 8, height = 6, units = "in")
#ggsave(filename = "../../../4.mSystems/2.revisionv2/figs/umap-ac-date_update.eps", umap_plot_date, width = 8, height = 6, units = "in")
#write.csv(log_exp, here::here("Dataset4-ibd","ac", "dataset4-ac-ms1_log2_normalized_update.csv"), quote=F, row.names = T)
```
...
kmedoid_labels <- read.csv(here::here("Dataset4-ibd", "ac","update", "k-med_labels.csv"), header=F) %>% as.vector()
...
clust_cols_ms1_svd <- c("#48392A", "#48392A", "black")

ac_ibd_dend <- color_branches(as.dendrogram(drug_clust_svd), k = 3, col = clust_cols_ms1_svd) %>%
    set("branches_lwd", 2)

k_3 <- cutree(ac_ibd_dend, k = 3, order_clusters_as_data = FALSE) 
...
ac_ibd_dend <- ac_ibd_dend %>% set_labels(., labels = c("Control", "CD (infl.)", "CD", 
                                                        "UC", "UC (inlf.)"))
                                            

#pdf(here::here("Dataset4-ibd", "ac", "update", "ibd-ac-dend.pdf"))
ac_ibd_dend %>% plot()
...
auto_feature_selection <- function(log_exp, kmed_labels, cond, cluster_cor, randomnumber=1234){
...
(CD_Y_roc <- ggroc(CD_Y_res$roc, colour = 'red', size = 1) +
  ggtitle(paste0('CD_Y; AUC = ', CD_Y_res$auc)) +
  geom_abline(intercept=1, slope=1, linetype="dashed") +
  theme_bw())
CD_Y_res$feat_plot

#write.csv(CD_Y_res$feat_rt_mz, here::here("Dataset4-ibd", "ac", "feat_selection", "CD_Y_mz-rt.csv"), row.names=F, quote =F)
#ggsave(here::here("Dataset4-ibd", "ac", "feat_selection", "CD_Y_feat.pdf"),CD_Y_res$feat_plot, width=7, height=7, units="in")
#ggsave(here::here("Dataset4-ibd", "ac", "feat_selection", "CD_Y_roc.pdf"), CD_Y_roc, width=7, height=7, units="in")
...
UC_Y_res <- auto_feature_selection(log_exp=log_exp, kmed_labels = kmedoid_labels, cond="UC_Yes",
                       cluster_cor = moduleTraitCor_svd)
#broom::tidy(UC_Y_res$final_model)
#UC_Y_res$roc
...
#write.csv(Control_res$feat_rt_mz, here::here("Dataset4-ibd", "ac", "feat_selection", "Ctrl_mz-rt.csv"), row.names=F, quote =F)
#ggsave(here::here("Dataset4-ibd", "ac", "feat_selection", "Ctrl_feat.pdf"), Control_res$feat_plot,
#       width=7, height=7, units="in")
#ggsave(here::here("Dataset4-ibd", "ac", "feat_selection", "Ctrl_roc.pdf"), Control_roc,width=7, height=7, units="in")
```
