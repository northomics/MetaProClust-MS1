...
Let's show how we can use Lasso to identify for features that can identify CD without inflammation.
...
(CD_Y_roc <- ggroc(CD_Y_res$roc, colour = 'red', size = 1) +
  ggtitle(paste0('CD (infl.); AUC = ', CD_Y_res$auc)) +
...
UC_Y_res$roc
UC_Y_res$feat_plot
(UC_Y_roc <- ggroc(UC_Y_res$roc, colour = 'red', size = 1) +
  ggtitle(paste0('UC (infl.); AUC = ', UC_Y_res$auc)) +
  geom_abline(intercept=1, slope=1, linetype="dashed") +
  theme_bw())

#ggsave(here::here("Dataset4-ibd", "dc", "feat_selection", "UC_Y_feat.pdf"), UC_Y_res$feat_plot)
#ggsave(here::here("Dataset4-ibd", "dc", "feat_selection", "UC_Y_roc.pdf"), UC_Y_roc)
#write.csv(UC_Y_res$feat_rt_mz, here::here("Dataset4-ibd", "dc", "feat_selection", "UC_Y_mz-rt.csv"), row.names=F, quote =F)
...
#write.csv(Control_res$feat_rt_mz, here::here("Dataset4-ibd", "dc", "feat_selection", "Ctrl_mz-rt.csv"), row.names=F, quote =F)
#ggsave(here::here("Dataset4-ibd", "dc", "feat_selection", "Ctrl_feat.pdf"), Control_res$feat_plot)
#ggsave(here::here("Dataset4-ibd", "dc", "feat_selection", "Ctrl_roc.pdf"), Control_roc)
```