...
ac_samples <- read.csv(here::here("ibd", "ac", "ac-samples.csv"), header=F) %>% 
    dplyr::select(-V1) %>%
    mutate_if(is.character, str_trim)
...
featuresibd <- read.csv(here::here("ibd", "ac", "ibd-nc-ac-ms1features-20mins-50pm.csv"))
...
(umap_plot_date <- ggplot(coords_umap, aes(x = X1, y = X2)) +
    ...
    labels = format(labels, "%m/%d/%y")
        ...
        theme_bw() +
        theme(legend.position = "right", text=element_text(size=16)))
#ggsave(filename = "../figs/umap-ac-diagnosis.pdf", umap_plot_diagnosis, width = 8, height = 6, units = "in")
#ggsave(filename = "../figs/umap-ac-date.pdf", umap_plot_date, width = 8, height = 6, units = "in")
```
...
all_samples_noctrl <- ac_metadata  %>% dplyr::filter(Diagnosis != "Control")# %>% 
    #mutate(Name = paste0(., "_", Groups))

fc_all <- fc_all %>% dplyr::select(all_samples_noctrl$NewName)
#write.csv(fc_all, here::here("ibd","ac", "nc-ibd_ms1_fc-acOnly.csv"), quote=F, row.names = T)
```
...
kmedoid_labels <- read.csv(here::here("ibd", "ac", "k-med_labels.csv"), header=F) %>% as.vector()
...
noctrlmeta <- ac_metadata %>% filter(Diagnosis != "Control")
...
#pdf(here::here("ibd", "ac", "ibd-dc-dend.pdf"))
ac_ibd_dend %>% plot()
