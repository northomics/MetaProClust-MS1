...
dc_samples <- read.csv(here::here("ibd", "dc", "dc-samples.csv"), header=F) %>% 
    dplyr::select(-V1) %>%
    mutate_if(is.character, str_trim)
...
featuresibd <- read.csv(here::here("ibd", "dc", "ibd-nc-dc-ms1features-20mins-50pm.csv"))
...
(umap_plot_date <- ggplot(coords_umap, aes(x = X1, y = X2)) +
    ...
    labels = format(labels, "%m/%d/%y")
        ...
        theme_bw() +
        theme(legend.position = "right", text=element_text(size=16)))
ggsave(filename = "../figs/umap-dc-diagnosis.pdf", umap_plot_diagnosis, width = 8, height = 6, units = "in")
ggsave(filename = "../figs/umap-dc-date.pdf", umap_plot_date, width = 8, height = 6, units = "in")
```
...
all_samples_noctrl <- dc_metadata  %>% dplyr::filter(Diagnosis != "Control")# %>% 
    #mutate(Name = paste0(., "_", Groups))

fc_all <- fc_all %>% dplyr::select(all_samples_noctrl$NewName)
#write.csv(fc_all, here::here("ibd","dc", "nc-ibd_ms1_fc-dcOnly.csv"), quote=F, row.names = T)
```
...
kmedoid_labels <- read.csv(here::here("ibd", "dc", "k-med_labels.csv"), header=F) %>% as.vector()
...
noctrlmeta <- dc_metadata %>% filter(Diagnosis != "Control")
...
dc_ibd_dend <- color_branches(as.dendrogram(drug_clust_svd), k = 2, col = clust_cols_ms1_svd) %>%
    set("branches_lwd", 2)

k_2 <- cutree(dc_ibd_dend, k = 2, order_clusters_as_data = FALSE) 
...
k_2 <- cutree(dc_ibd_dend, k = 2, order_clusters_as_data = FALSE) 
dc_ibd_dend <- dc_ibd_dend %>% set_labels(., labels = c("CD_inf", "UC", "CD", "UC_inf"))
#(here::here("ibd", "dc", "ibd-dc-dend.pdf"))
dc_ibd_dend %>% plot()
